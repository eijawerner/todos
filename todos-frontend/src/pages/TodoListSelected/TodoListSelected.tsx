...
const TodoListSelectedUnstyled = ({ className }: TodoListProps) => {
    ...
    return (
        <div className={className}>
            <TodoListCreateForm isVisible={newListFormIsVisible} onCreateTodoList={handleCreateTodoList} onCloseOverlayClick={handleCloseOverlayClick} />

            <div style={{ display: 'flex', flexDirection: 'row', padding: '15px', gap: '5px', alignItems: 'center'}}>
                <TodoListSelector selected={selectedList} todoLists={todoLists} onSelectTodoListChange={handleSelectTodoList} />
                <Button appearance='secondary' onClick={handleOpenNewListForm} text={"New list"} size={'small'} />
            </div>
